...
class CNN_conv(nn.Module):
    def __init__(self, in_ch: int, num_cls: int, h: int, w: int):
        ...
        self.conv3 = ConvConvReLU(32, 64, 5)
        self.maxpool3 = nn.MaxPool2d(2)
        self.h, self.w = self.h // 2, self.w // 2
        
        self.conv4 = ConvConvReLU(64, 128, 5)
        self.maxpool4 = nn.MaxPool2d(2)
        self.h, self.w = self.h // 2, self.w // 2
...
class CNN_conv_bone(nn.Module):
    def __init__(self, in_ch: int, num_cls: int, h: int, w: int):
        ...
        self.fc1 = nn.Linear(256 * self.h * self.w, 128)
        self.relu = nn.ReLU()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.fc3(self.relu2(self.fc2(self.relu(self.fc1(x)))))

        return x

class CNN_BN_bone(nn.Module):
    def __init__(self, in_ch: int, num_cls: int, h: int, w: int):
        super().__init__()
        self.h = h
        self.w = w
